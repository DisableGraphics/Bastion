...
FAT32::FAT32(PartitionManager &partmanager, size_t partid) {
	...
	if(job->state == DiskJob::FINISHED) {
		...
		auto total_sectors = (fat_boot->total_sectors_16 == 0)? fat_boot->total_sectors_32 : fat_boot->total_sectors_16;
		auto fat_size = (fat_boot->table_size_16 == 0)? fat_boot_ext_32->table_size_32 : fat_boot->table_size_16;
		char name[12];
		memcpy(name, fat_boot_ext_32->volume_label, 11);
		name[11] = 0;
		printf("Total sectors: %d. FAT Size: %d. Name: %s\n", total_sectors, fat_size, name);
	}
